...
#workTabu .tabulator-row.tabulator-row-even {
...
/* 단취작업 등록 모달의 *적재방법* 영역처럼 j_row1이 여러 줄 누적되는 곳은
   가로 flex 배치 대신 세로로 쌓이도록 강제 (그 외 j_row1은 단일행/타이틀용) */
.j_container.j_container_stack {
	flex-direction: column;
	align-items: stretch;
}
.j_container.j_container_stack .j_row1 {
	width: 100%;
}
.daylabel {
	font-size: 12px;
...
/* nbsp/margin_left로 간격 맞추던 옛 방식 대체 (gap이 대신 처리) */
.margin_left {
...
/* 단취-입고이력 모달은 단취모달 위에 얹히므로 z-index를 한 단계 더 높게 */
.workDanIpgoModal {
	z-index: 20011;
}
.header {
...
/* ========== 모달 내 리스트 (workDanTabu / workDanIpgoTabu / workBcfTabu / workTfTabu) ========== */
...
#workDanTabu .tabulator-row.row_select,
#workDanIpgoTabu .tabulator-row.row_select,
#workBcfTabu .tabulator-row.row_select,
#workTfTabu .tabulator-row.row_select {
    background-color: #BEE3F8 !important;
    box-shadow: inset 0 0 0 2px #2B6CB0;
}
    ...
    <div class="box1">
		...
		<form action="" autocomplete="off">
			<div class="j_container">
				...
				<input type="date" id="s_all_sdate" class="datetimepicker_date">
				~
				<input type="date" id="s_all_edate" class="datetimepicker_date">
...
<main class="main">
	<div class="container">
		<div id="workTabu" class="tabulator"></div>
	</div>
</main>
...
<div class="workSelectModal">
    ...
    <div class="btnSaveClose">
		<button class="close" type="button" onclick="workSelectModalClose();">닫기</button>
    </div>
</div>
...
<div class="workDanModal">
	<div class="detail">
		...
		<div class="j_container j_container_stack">
			<div class="j_row1">
				<div class="j_div">
...
<script>
	...
	$("#workDeleteBtn").on("click", function(){
		...
		if(confirm("작업/단취코드 : "+selectedData.ilbo_code+"\n수주번호 : "+selectedData.ord_code+"\n작업구분 : "+selectedData.ilbo_gubn+"\n작업LOT : "+selectedData.ilbo_lot+"\n를 삭제하시겠습니까?")){
			...
			$.ajax({
				url:"/tkheat/production/workInstructionTk/dataDelete",
				type:"post",
				dataType:"json",
				data:{
					"ilbo_code":ilbo_code,
	...
	function ilboLotRtn(facCodeParam){
		...
		$.ajax({
			url:"/tkheat/production/workInstructionTk/bcf/ilboLotRtn",
			type:"post",
			dataType:"json",
			data:{
				"ilbo_lot_date":(ye+"-"+mo+"-"+da),
				"fac_code":facCodeParam,
				"ilbo_code":workDataTable.getSelectedData()[0].ilbo_code
			},
			success:function(result){
	...
	function workSelectModalOpen(){		
		workSelectModal.style.display = 'flex'; // 모달 표시
	}
	...
	function workDanModalOpen(){
		...
		workDanModal.style.display = 'flex'; // 모달 표시
	}	
	...
	function workBcfModalOpen(){
		...
		if(selectedData == null){
			...
			if(ilbo_gubn == "J" || ilbo_gubn == "A"){
				...
				workBcfModal.style.display = 'flex'; // 모달 표시
				
				var selectedDataParam = JSON.stringify(selectedData);
				
				$.ajax({
					url:"/tkheat/production/workInstructionTk/bcf/bcfDataSearch",
					type:"post",
					dataType:"json",
					traditional: true,
...
//						var bcf_total_weight = 0;	//총 중량
						...
						workTfModal.style.display = 'flex';
					}
				});
	...
	function workDanIpgoModalOpen(){
		...
		getWorkDanIpgoData();
		getWorkDanIpgoDataList();
		workDanIpgoModal.style.display = 'flex'; // 모달 표시
	}
...
//		user_code
	...
	function getWorkDanIpgoDataBarcodeScan(){
		...
		$.ajax({
			url:"/tkheat/production/workInstructionTk/dan/ipgoBarcodeScan",
			type:"post",
			dataType:"json",
			data:{
				"ord_code":barcode
...
//		user_code
	...
	function workBcfModalSave(){
		...
		$.ajax({
			url:"/tkheat/production/workInstructionTk/bcf/dataSave",
			type:"post",
			dataType:"json",
			traditional: true,
			data:{
