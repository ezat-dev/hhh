...
/* ========== 치수 측정값 MIN/MAX 범위 벗어남 표시 ========== */
.small-input.dim-invalid {
    background-color: #ff6b6b !important;
    color: white !important; border-color: #fa5252 !important;
}
...
function checkAutoJudge(){
    ...
    let hasDimInvalid = $(".small-input.dim-invalid").length > 0;

    if(hasNG || hasDimInvalid){
        $("#itst_wp").val("불합격");
    } else {
        // 전부 OK면 합격으로 복원
        $("#itst_wp").val("합격");
...
function applyNgStyle(){
    ...
    validateAllDimInputs(); // ★ 저장된 데이터 로드 시 치수 범위 벗어남 표시도 복원
}

//===== 치수1~5 측정값 MIN/MAX 범위 검증 =====
function validateDimInput($input){
    var $fields = $input.closest(".inspection-fields");
    var min = parseFloat($fields.find('input[type="text"]:first-child').val());
    var max = parseFloat($fields.find('input[type="text"]:nth-child(2)').val());
    var val = parseFloat($input.val());

    var hasRange = !isNaN(min) && !isNaN(max);
    var isInvalid = hasRange && !isNaN(val) && (val < min || val > max);

    $input.toggleClass("dim-invalid", isInvalid);
}

function validateAllDimInputs(){
    $(".inspection-fields .small-input").each(function(){
        validateDimInput($(this));
    });
}

// MIN/MAX 또는 측정값(치수1~5) 입력 시 실시간으로 범위 검증 + 최종판정 갱신
$(document).on("input", ".inspection-fields input[type='text']", function(){
    var $fields = $(this).closest(".inspection-fields");
    if($fields.find(".small-input").length > 0){
        $fields.find(".small-input").each(function(){
            validateDimInput($(this));
        });
        checkAutoJudge();
    }
...
insertButton.addEventListener('click', function() {
    isEditMode = false;
    $('#suipForm')[0].reset();
    $(".ok-ng-select").removeClass("ng-selected");
    $(".small-input").removeClass("dim-invalid");
