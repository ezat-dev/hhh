...
@Service
public class MonitoringServiceBcf2Impl implements MonitoringServiceBcf2{
...
//		long resetValue = 0;
		...
		Map<String, Object> qrMap = opcData.getOpcData("TKHEAT.MODBUS.MONITORING.QR_BCF2");
		Map<String, Object> qrBuffMap = opcData.getOpcData("TKHEAT.MODBUS.MONITORING.QR_BUFF_BCF2");
		
		int qr = Integer.parseInt(qrMap.get("value").toString());
		int qrBuff = Integer.parseInt(qrBuffMap.get("value").toString());		

		Map<String, Object> preMap = opcData.getOpcData("TKHEAT.MODBUS.MONITORING.PRE_BCF2");		
		Map<String, Object> chimMap = opcData.getOpcData("TKHEAT.MODBUS.MONITORING.CHIM_BCF2");
		Map<String, Object> diffMap = opcData.getOpcData("TKHEAT.MODBUS.MONITORING.DIFF_BCF2");
		Map<String, Object> gangMap = opcData.getOpcData("TKHEAT.MODBUS.MONITORING.GANG_BCF2");
		Map<String, Object> coldMap = opcData.getOpcData("TKHEAT.MODBUS.MONITORING.COLD_BCF2");
		Map<String, Object> chulMap = opcData.getOpcData("TKHEAT.MODBUS.MONITORING.CHUL_BCF2");
		...
		if(qrBuff != 0) {
			...
			//불일치시 미적용
			if("2".equals((qrBuff+"").substring(6,7))){
				if(qr == 0) {
					//실 적용 바코드의 값이 0이면 버퍼의 값을 실제 적용 바코드로 값 쓰기.
					opcData.setOpcData("TKHEAT.MODBUS.MONITORING.WRITE_BUFF_BCF2", true);
				}				
			}else {
				//해당 호기가 아닐경우 값 리셋
				opcData.setOpcData("TKHEAT.MODBUS.MONITORING.RESET_BUFF_BCF2", true);
			}
...
//					//System.out.println("출구만 신호 들어옴 : "+chul+"// : "+qr);
	...
	//					chulVirt = false;
					...
					while(v1Chk) {
						if(qr != 0) {
							opcData.setOpcData("TKHEAT.MODBUS.MONITORING.RESET_BCF2",true);
							v1Chk = false;
						}
					}
					chulVirt = false;
...
//			chulVirt = true;
	...
	//출구이후 사용
	/*
	Map<String, Object> v1ChkMap = opcData.getOpcData("TKHEAT.MODBUS.MONITORING.QR_BCF2");
