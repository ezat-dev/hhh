...
@Service
public class MonitoringServiceBcf1Impl implements MonitoringServiceBcf1{
...
//		long resetValue = 0;
		...
		Map<String, Object> qrBuffMap = opcData.getOpcData("TKHEAT.MODBUS.MONITORING.QR_BUFF_BCF1");
		...
		Map<String, Object> chimMap = opcData.getOpcData("TKHEAT.MODBUS.MONITORING.CHIM_BCF1");
		Map<String, Object> diffMap = opcData.getOpcData("TKHEAT.MODBUS.MONITORING.DIFF_BCF1");
		Map<String, Object> gangMap = opcData.getOpcData("TKHEAT.MODBUS.MONITORING.GANG_BCF1");
		Map<String, Object> coldMap = opcData.getOpcData("TKHEAT.MODBUS.MONITORING.COLD_BCF1");
		Map<String, Object> chulMap = opcData.getOpcData("TKHEAT.MODBUS.MONITORING.CHUL_BCF1");
		...
		if(qrBuff != 0) {
			...
			if("1".equals((qrBuff+"").substring(6,7))){
				if(qr == 0) {
					...
					opcData.setOpcData("TKHEAT.MODBUS.MONITORING.WRITE_BUFF_BCF1", true);
				}				
			}else {
				//해당 호기가 아닐경우 값 리셋
				opcData.setOpcData("TKHEAT.MODBUS.MONITORING.RESET_BUFF_BCF1", true);
			}
...
//					System.out.println("출구만 신호 들어옴 : "+chul+"// : "+qr);
	...
	//					chulVirt = false;
					...
					while(v1Chk) {
						if(qr != 0) {
							opcData.setOpcData("TKHEAT.MODBUS.MONITORING.RESET_BCF1",true);
							v1Chk = false;
						}
					}
					chulVirt = false;
...
//			chulVirt = true;
	...
	//출구이후 사용
	/*
	Map<String, Object> v1ChkMap = opcData.getOpcData("TKHEAT.MODBUS.MONITORING.QR_BCF1");
